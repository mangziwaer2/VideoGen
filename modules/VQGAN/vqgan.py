...
if __name__ == '__main__':
    vq=VQModel(n_embed=8192)
    input=torch.randn((1,3,72,128))
    in_img=input.squeeze(0).permute(1,2,0).cpu().numpy()
    out=vq(input)

    print(out[0].shape)
    print(out[1])
    out_img=out[0].squeeze(0).permute(1,2,0).detach().cpu().numpy()
